...
class RegisterHandler:
    ...
    async def __call__(self, request):
        ...
        mode=self.detector.detect(payload.get("console_type"), upstream); provider=payload.get("provider") or "gcore"
        ...
        return web.json_response({"token":token,"url":public,"ttl":self.config.session_ttl_seconds,"type":mode})